...
def read_comments_for_blog_post(session: SessionDep, blog_post_id: int, skip: int = 0, limit: int = 100) -> CommentsPublic:
    """
    ...
    count, comments = CommentCRUD(session).read_comments_for_blog_post(blog_post_id=blog_post_id, skip=skip, limit=limit)
    comments = [CommentPublicWithUsername(id=comment.id,
...
def read_comment(session: SessionDep, id: int) -> CommentPublicWithUsername:
    ...
    return CommentPublicWithUsername(id=comment.id,
                                     content=comment.content,
                                     comment_date=comment.comment_date,
                                     user_id=comment.user_id,
                                     blog_post_id=comment.blog_post_id,
...
@router.post("/blogpost/{blog_post_id}/comments", response_model=CommentPublicWithUsername)
def create_comment(session: SessionDep, blog_post_id: int, comment_in: CommentCreate, 
                   current_user: CurrentUser) -> CommentPublicWithUsername:
    """
    ...
    comment = CommentCRUD(session).create_comment(comment=comment_in, user_id=current_user.id, blog_post_id=blog_post_id)
    return CommentPublicWithUsername(id=comment.id,
                                     content=comment.content,
                                     comment_date=comment.comment_date,
                                     user_id=comment.user_id,
...
def update_comment_on_blog_post(session: SessionDep, blog_post_id: int, id: int, comment_in: CommentUpdate, 
    ...
    if comment.user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No permission to update this comment")
    ...
    db_comment = CommentCRUD(session).update_comment(comment_db=comment, comment_in=comment_in)
    return db_comment


@router.delete("/blogpost/{blog_post_id}/comments/{id}", response_model=Message)
def delete_comment_on_blog_post(session: SessionDep, blog_post_id: int, id: int, current_user: CurrentUser) -> Message:
    """
    Delete a comment.
    """
    blog_post = session.get(BlogPost, blog_post_id)
