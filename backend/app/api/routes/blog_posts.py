...
from app.schemas.blog_post import BlogPostPublic, BlogPostCreate, BlogPostUpdate, BlogPostsPublic, BlogPostPublicWithComments
...
def read_blog_posts(session: SessionDep, skip: int = 0, limit: int = 100) -> BlogPostsPublic:
    """
    Retrieve blog posts.
    """
...
def read_blog_post(session: SessionDep, id: int) -> BlogPostPublicWithComments:
    ...
    """
    blog_post = BlogPostCRUD(session).read_blog_post_with_comments_and_tags(blog_post_id=id)
    if not blog_post:
    ...
    tags = [TagPublic.model_validate(tag, from_attributes=True) for tag in blog_post.tags]

    return BlogPostPublicWithComments(id=blog_post.id,
...
def create_blog_post(session: SessionDep, blog_post_in: BlogPostCreate) -> BlogPostPublic:
    """
...
def update_blog_post(session: SessionDep, id: int, blog_post_in: BlogPostUpdate) -> BlogPostPublic:
    ...
    blog_post_crud = BlogPostCRUD(session)
    if blog_post.title != blog_post_in.title and blog_post_crud.get_blog_post_by_title(blog_title=blog_post_in.title):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="A blog post with this title already exists",
        )
    if blog_post.url != blog_post_in.url and blog_post_crud.get_blog_post_by_url(blog_url=blog_post_in.url):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="A blog post with this url already exists",
        )
    
    db_blog_post = BlogPostCRUD(session).update_blog_post(blog_post_db=blog_post, blog_post_in=blog_post_in)
    return db_blog_post
