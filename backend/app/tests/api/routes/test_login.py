...
def test_05_get_access_token_user(client: TestClient, setup_user: tuple[str, str]) -> None:
...
def test_07_login_invalidpassword(client: TestClient, setup_user: tuple[str, str]) -> None:
    username, password = setup_user
    login_data = {
        "username": username,
...
def test_08_login_inactive_user(client: TestClient, db: Session, setup_user: tuple[str, str]) -> None:
    username, password = setup_user
    user_crud = UserCRUD(db)
...
def test_09_activate_user(client: TestClient, db: Session, setup_user: tuple[str, str]) -> None:
    # Deactivate the user
    email, _ = setup_user
    ...
    # Activate the user
    response = client.get(f"{settings.API_VERSION_STR}/users/activate?token={activation_token}")
    assert response.status_code == 200
    data = response.json()
...
def test_10_activate_user_already_active(client: TestClient, setup_user: tuple[str, str]) -> None:
    email, _ = setup_user
    activation_token = generate_token(email)
    response = client.get(f"{settings.API_VERSION_STR}/users/activate?token={activation_token}")
    assert response.status_code == 200
...
def test_12_forgot_password(client: TestClient, setup_user: tuple[str, str]) -> None:
    ...
    with patch.object(RolkoTechEmail, "send", return_value=None) as mock_send:
        response = client.post(f"{settings.API_VERSION_STR}/users/forgot-password?email={email}")
        mock_send.assert_called_once()
    message = response.json()
    assert response.status_code == 200
    assert message["message"] == "If the email exists and is active, a reset link has been sent."


def test_13_forgot_password_inactive_user(client: TestClient, db: Session, setup_user: tuple[str, str]) -> None:
    email, _ = setup_user
    user_db = UserCRUD(db).get_user_by_email(email=email)
    user_db.is_active = False
    db.add(user_db)
    ...
    with patch.object(RolkoTechEmail, "send", return_value=None) as mock_send:
        response = client.post(f"{settings.API_VERSION_STR}/users/forgot-password?email=invalid_email")
...
def test_15_password_reset(client: TestClient, setup_user: tuple[str, str]) -> None:
    ...
    response = client.post(f"{settings.API_VERSION_STR}/users/reset-password",
...
def test_19_password_reset_invalid_new_password(client: TestClient, setup_user: tuple[str, str]) -> None:
    email, _ = setup_user
    # Generate a token for the user
    token = generate_token(email)
