...
def test_03_read_blog_posts_with_skip_and_limit(client: TestClient, db: Session) -> None:
    blog_post_1 = BlogPostCRUD(db).create_blog_post(
        blog_post=BlogPostCreate(title="Blog Post 1",
                                 url="blog-post-1",
                                 content="Content of Blog Post 1",
                                 image_path="image.png",
                                 tags=[]))
    blog_post_2 = BlogPostCRUD(db).create_blog_post(
        blog_post=BlogPostCreate(title="Blog Post 2",
                                 url="blog-post-2",
                                 content="Content of Blog Post 2",
                                 image_path="image.png",
                                 tags=[]))
    blog_post_3 = BlogPostCRUD(db).create_blog_post(
        blog_post=BlogPostCreate(title="Blog Post 3",
                                 url="blog-post-3",
...
def test_05_read_blog_post_with_tags_and_comments(client: TestClient, 
                                                  setup_blog_post_with_tag_and_comment: BlogPost) -> None:
...
def test_07_create_blog_post(client: TestClient, superuser_token_headers: dict[str, str]) -> None:
    blog_post_data = {
        "title": "Blog Post 1",
        "url": "blog-post-1",
        "content": "Content of Blog Post 1"
    }
    response = client.post(f"{settings.API_VERSION_STR}/blogposts/", headers=superuser_token_headers,
                           json=blog_post_data)
    assert response.status_code == 200
    data = response.json()
...
def test_08_create_blog_post_with_tags(client: TestClient, setup_tag: Tag, 
...
def test_09_create_blog_post_with_existing_title(client: TestClient, setup_blog_post: BlogPost,
                                                 superuser_token_headers: dict[str, str]) -> None:
    blog_post_data = {
...
def test_10_create_blog_post_with_existing_url(client: TestClient, setup_blog_post: BlogPost,
                                               superuser_token_headers: dict[str, str]) -> None:
    blog_post_data = {
        "title": "Blog Post 999",
...
def test_11_create_blog_post_unauthorized(client: TestClient, 
    ...
    response = client.post(f"{settings.API_VERSION_STR}/blogposts/", headers=normal_user_token_headers,
                           json=blog_post_data)
...
def test_12_create_blog_post_invalid_title(client: TestClient, 
                                           superuser_token_headers: dict[str, str]) -> None:
    blog_post_data = {
        "title": "",
        "url": "blog-post-1",
...
def test_13_create_blog_post_invalid_url(client: TestClient, 
                                         superuser_token_headers: dict[str, str]) -> None:
    blog_post_data = {
        "title": "Blog Post 1",
        "url": "",
...
def test_14_update_blog_post(client: TestClient, setup_blog_post: BlogPost,
                             superuser_token_headers: dict[str, str]) -> None:
    blog_post_data = {
    ...
    response = client.patch(f"{settings.API_VERSION_STR}/blogposts/{setup_blog_post.id}", 
                            headers=superuser_token_headers, json=blog_post_data)
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == setup_blog_post.id
...
def test_15_update_blog_post_everything(client: TestClient, setup_blog_post: BlogPost, setup_tag: Tag,
                                        superuser_token_headers: dict[str, str]) -> None:
    blog_post_data = {
        "title": "Updated Blog Post",
        "url": "updated-blog-post",
...
def test_16_update_blog_post_not_found(client: TestClient, superuser_token_headers: dict[str, str]) -> None:
    blog_post_data = {
        "title": "Updated Blog Post",
        "url": "updated-blog-post",
        "content": "Updated content of Blog Post"
    }
    response = client.patch(f"{settings.API_VERSION_STR}/blogposts/999", 
                            headers=superuser_token_headers, json=blog_post_data)
    assert response.status_code == 404
    data = response.json()
    assert data["detail"] == "Blog post not found"
...
def test_17_update_blog_post_with_existing_title(client: TestClient, db: Session, setup_blog_post: BlogPost,
                                                 superuser_token_headers: dict[str, str]) -> None:
    ...
    response = client.patch(f"{settings.API_VERSION_STR}/blogposts/{blog_post.id}", 
                            headers=superuser_token_headers, json=blog_post_data)
    assert response.status_code == 400
    data = response.json()
    assert data["detail"] == "A blog post with this title already exists"
...
def test_18_update_blog_post_with_existing_title(client: TestClient, db: Session, setup_blog_post: BlogPost,
                                                 superuser_token_headers: dict[str, str]) -> None:
    blog_post = BlogPostCRUD(db).create_blog_post(
        blog_post=BlogPostCreate(title="Blog Post 2",
                                 url="blog-post-2",
...
def test_19_update_blog_post_unauthorized(client: TestClient, setup_blog_post: BlogPost,
                                          normal_user_token_headers: dict[str, str]) -> None:
    blog_post_data = {
        "title": "Updated Blog Post",
        "url": "updated-blog-post",
    ...
    response = client.patch(f"{settings.API_VERSION_STR}/blogposts/{setup_blog_post.id}", 
                            headers=normal_user_token_headers, json=blog_post_data)
    assert response.status_code == 403
    data = response.json()
    assert data["detail"] == "The user does not have admin privileges"


def test_20_delete_blog_post(client: TestClient, setup_blog_post: BlogPost,
                             superuser_token_headers: dict[str, str]) -> None:
    response = client.delete(f"{settings.API_VERSION_STR}/blogposts/{setup_blog_post.id}", 
...
def test_21_delete_blog_post_not_found(client: TestClient, superuser_token_headers: dict[str, str]) -> None:
    response = client.delete(f"{settings.API_VERSION_STR}/blogposts/999", 
                             headers=superuser_token_headers)
    assert response.status_code == 404
    data = response.json()
    assert data["detail"] == "Blog post not found"
...
def test_22_delete_blog_post_unauthorized(client: TestClient, setup_blog_post: BlogPost,
                                          normal_user_token_headers: dict[str, str]) -> None:
    response = client.delete(f"{settings.API_VERSION_STR}/blogposts/{setup_blog_post.id}", 
...
def test_23_delete_blog_post_with_comments(client: TestClient, setup_blog_post_with_tag_and_comment: BlogPost,
                                           superuser_token_headers: dict[str, str]) -> None:
    blog_post, tag, comment = setup_blog_post_with_tag_and_comment
    response = client.delete(f"{settings.API_VERSION_STR}/blogposts/{blog_post.id}", 
                             headers=superuser_token_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["message"] == "Blog post deleted successfully"
