...
class UserCRUD(BaseCRUD):
    ...
    def create_user(self, user: UserCreate | UserRegister) -> User:
        user = self.MODEL_CLASS.model_validate(
            user, update={"password": get_password_hash(user.password.get_secret_value())}
        )
    ...
    def read_tag_with_blog_posts(self, tag_id: int) -> Tag:
        statement = (
            select(self.MODEL_CLASS)
                .options(
                    joinedload(self.MODEL_CLASS.blog_posts)
                )
...
class BlogPostCRUD(BaseCRUD):
    ...
    def read_blog_post_with_comments_and_tags(self, blog_post_id: int) -> BlogPost:
        ...
        select(self.MODEL_CLASS)
            .options(
                joinedload(self.MODEL_CLASS.comments),
                joinedload(self.MODEL_CLASS.tags)
            )
    ...
    def update_blog_post(self, blog_post_db: BlogPost, blog_post_in: BlogPostUpdate) -> BlogPost:
...
class CommentCRUD(BaseCRUD):
    ...
    def create_comment(self, comment: CommentCreate, user_id: uuid.UUID, blog_post_id: int) -> Comment:
    ...
    def read_comments_with_username(self, skip: int, limit: int) -> tuple[int, list[Comment]]:    
        count_statement = select(func.count()).select_from(self.MODEL_CLASS)
        count = self.session.exec(count_statement).one()
    ...
    def read_comments_for_blog_post(self, blog_post_id: int, skip: int, limit: int) -> tuple[int, list[Comment]]:
        count_statement = select(func.count()).select_from(self.MODEL_CLASS)
        count = self.session.exec(count_statement).one()

        statement = select(self.MODEL_CLASS).where(self.MODEL_CLASS.blog_post_id == blog_post_id).offset(skip).limit(limit)
        objects = self.session.exec(statement).all()

        return count, objects
    
    def read_comments_for_user(self, user_id: uuid.UUID, skip: int, limit: int) -> tuple[int, list[Comment]]:
        count_statement = select(func.count()).select_from(self.MODEL_CLASS)
        count = self.session.exec(count_statement).one()

        statement = select(self.MODEL_CLASS).where(self.MODEL_CLASS.user_id == user_id).offset(skip).limit(limit)
        objects = self.session.exec(statement).all()
